...
describe("On-chain proxy helpers", function () {
  ...
  for (const withCall of [false, true]) {
    it(`executes a prepared admin upgrade ${
      withCall ? "with" : "without"
    } a call`, async function () {
      const { helpers, ethers, upgrades } = this.hre
      const [owner, stranger] = await ethers.getSigners()
      const [, original] = await helpers.upgrades.deployProxy("Counter", {
        initializerArgs: [42],
      })
      const before = savedRecord()
      const factory = await ethers.getContractFactory("CounterV2")
      const { preparedTransaction, newImplementationAddress } =
        await helpers.upgrades.prepareProxyUpgrade("Counter", "CounterV2", {
          callData: withCall
            ? factory.interface.encodeFunctionData("setExtra", [7])
            : undefined,
        })
      expect(preparedTransaction.from).to.equal(await owner.getAddress())
      expect(
        await upgrades.erc1967.getImplementationAddress(original.address)
      ).to.equal(original.implementation)
      expect(savedRecord()).to.equal(before)
      ...
      expect(savedRecord()).to.equal(before)
      const tx = await owner.sendTransaction(preparedTransaction)
      expect((await tx.wait())!.status).to.equal(1)
      const upgraded = factory.attach(original.address)
      expect((await upgraded.value()).toString()).to.equal("42")
      expect((await upgraded.extra()).toString()).to.equal(withCall ? "7" : "0")
      expect(
        await upgrades.erc1967.getImplementationAddress(original.address)
      ).to.equal(newImplementationAddress)
      // The owner executes independently; preparation never predicts execution.
      expect(savedRecord()).to.equal(before)
    })
  }

  it("keeps the record and implementation after a prepared call reverts", async function () {
    const { helpers, ethers, upgrades } = this.hre
    const [owner] = await ethers.getSigners()
    const [, original] = await helpers.upgrades.deployProxy("Counter", {
      initializerArgs: [42],
    ...
    const before = savedRecord()
    const factory = await ethers.getContractFactory("CounterV2")
    const { preparedTransaction } = await helpers.upgrades.prepareProxyUpgrade(
      "Counter",
      "CounterV2",
      {
        callData: factory.interface.encodeFunctionData("setExtra", [0]),
      }
    )
    await expect(owner.sendTransaction(preparedTransaction)).to.be.rejectedWith(
      "invalid extra"
    )
    expect(
      await upgrades.erc1967.getImplementationAddress(original.address)
  ...
  it("rejects preparing an incompatible storage layout without saving it", async function () {
    const { helpers } = this.hre
    await helpers.upgrades.deployProxy("Counter", { initializerArgs: [42] })
    const before = savedRecord()
    await expect(
      helpers.upgrades.prepareProxyUpgrade("Counter", "IncompatibleCounter")
    ).to.be.rejectedWith("incompatible")
    expect(savedRecord()).to.equal(before)
  })
})
